...
class NoteAPI(MethodView):
    """Generic API for manipulating notes"""

    def get(self, uuid):
        """Get note details (formatted as JSON) by note UUID."""
        try:
            nt = db.note_for_uuid(g.db, str(uuid))
...
app.add_url_rule('/api/note/<uuid:uuid>', view_func=note_view, methods=['GET', 'PATCH', 'DELETE'])
...
def add_notebook():
    ...
    notebook = request.form.get('name')
